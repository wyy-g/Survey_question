...
// 更新问卷信息
exports.updateQuestionModel = async (quesId, title, isStar, isPublished, isDeleted, description, isShowOrderIndex, updatedAt) => {
    return await executeQuery(
        `UPDATE surveys 
        SET title = ?, isStar = ?, isPublished = ?, isDeleted = ?, description = ?,isShowOrderIndex = ?, updatedAt = ?
        WHERE id = ?;`,
        [title, isStar, isPublished, isDeleted, description, isShowOrderIndex, updatedAt, quesId]
    )
}
