...
// 更新问卷信息
exports.updateQuestionModel = async (quesId, title, isStar, isPublished, isDeleted, description, isShowOrderIndex, updatedAt, startTime, endTime, isEnableFeedback, isMultiLang, lang, defaultLang) => {
    return await executeQuery(
        `UPDATE surveys 
        SET title = ?, isStar = ?, isPublished = ?, isDeleted = ?, description = ?,isShowOrderIndex = ?, updatedAt = ?, startTime = ?, endTime = ?, isEnableFeedback = ?, isMultiLang = ?, lang = ?, defaultLang = ?
        WHERE id = ?;`,
        [title, isStar, isPublished, isDeleted, description, isShowOrderIndex, updatedAt, startTime, endTime, isEnableFeedback, isMultiLang, lang, defaultLang, quesId]
    )
}
